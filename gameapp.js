...
let gesturePredicted = "forward";
let gesturePredicted_prev = "forward";
...
async function initML() {
	  // const modelURL = "./assets/ml-model/model.json";
	  // const metadataURL = "./assets/ml-model/metadata.json";
	  ...
	  // screenShot = await webcam.capture();
	  // screenShot.dispose();
...
async function predict() {
	...
	webcam.update();
	ctx.drawImage(webcam.canvas, 0, 0, 100, 100);
	...
	gesturePredicted = prediction.find(o => o.probability === maxGesture).className;
	console.log("Runing to: " + gesturePredicted);

	// // console.log("Start Sleep 500ms!");
	// if (gesturePredicted_prev == gesturePredicted) {
	// 	squirrel.allowLeft = true;
	// 	squirrel.allowRight = true;
	// } else {
	// 	switch (gesturePredicted) {
	// 		case "up":
	// 			if (gesturePredicted !== "") {
	// 				setTimeout(() => {squirrel.jump();}, timeGap);
	...
	// 		case "down":
	// 			if (gesturePredicted !== "")  {
	// 				setTimeout(() => {squirrel.duck();}, timeGap);
	// 				if (firstRun && hud.tutState == 1) {
	// 					hasVertAction = true;
	// 				}
	// 			}
	...
	// 		case "right":
	// 			if (gesturePredicted !== "")  {
	// 				setTimeout(() => {squirrel.turn("right");}, timeGap);
	...
	// 		case "left":
	// 			if (gesturePredicted !== "")  {
	// 				setTimeout(() => {squirrel.turn("left");}, timeGap);
	// 				if (firstRun && hud.tutState == 0) {
	// 					hasHorizAction = true;
	// 				}
	// 			} else {
	...
	switch (gesturePredicted) {
		...
		case "forward":
			if (gesturePredicted !== "") {
				squirrel.allowRight = false;
				squirrel.allowLeft = false;
...
(function(s) {
	var A = (function() {
		...
		// ctx.drawImage(image_data_url, 0, 0);
	...
	if (n6Z === n6Z) {
		var k = (function(s) {
			...
			A.prototype.update = function() {
				...
				this.fps = branch.scrollSpeed / 64;
				this.y += this.incY;
